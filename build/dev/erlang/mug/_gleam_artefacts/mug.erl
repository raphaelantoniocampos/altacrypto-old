...
-spec unsafe_coerce_packet(fun((tcp_message()) -> DKY)) -> fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> DKY).
...
-spec unsafe_coerce_closed(fun((tcp_message()) -> DKZ)) -> fun((gleam@dynamic:dynamic_()) -> DKZ).
unsafe_coerce_closed(Mapper) ->
    fun(Socket) -> _pipe = {socket_closed, unsafe_coerce_to_socket(Socket)},
        Mapper(_pipe) end.

-spec unsafe_coerce_to_tcp_error(fun((tcp_message()) -> DLA)) -> fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> DLA).
...
-spec selecting_tcp_messages(
    gleam@erlang@process:selector(DKV),
    fun((tcp_message()) -> DKV)
) -> gleam@erlang@process:selector(DKV).
