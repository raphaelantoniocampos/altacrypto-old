...
-spec set_header(request(LII), binary(), binary()) -> request(LII).
...
-spec set_query(request(LJE), list({binary(), binary()})) -> request(LJE).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(LJI), gleam@http:method()) -> request(LJI).
...
-spec set_scheme(request(LJP), gleam@http:scheme()) -> request(LJP).
set_scheme(Req, Scheme) ->
    erlang:setelement(5, Req, Scheme).

-spec set_host(request(LJS), binary()) -> request(LJS).
set_host(Req, Host) ->
    erlang:setelement(6, Req, Host).

-spec set_port(request(LJV), integer()) -> request(LJV).
set_port(Req, Port) ->
    erlang:setelement(7, Req, {some, Port}).

-spec set_path(request(LJY), binary()) -> request(LJY).
set_path(Req, Path) ->
    erlang:setelement(8, Req, Path).

-spec set_cookie(request(LKB), binary(), binary()) -> request(LKB).
