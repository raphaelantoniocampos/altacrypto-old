...
-spec map_response_body(
    fun((LTP) -> gleam@http@response:response(LTB)),
    fun((LTB) -> LTC)
) -> fun((LTP) -> gleam@http@response:response(LTC)).
...
-spec prepend_response_header(
    fun((LTT) -> gleam@http@response:response(LTV)),
    binary(),
    binary()
) -> fun((LTT) -> gleam@http@response:response(LTV)).
...
-spec ensure_post(gleam@http@request:request(LTG)) -> {ok,
        gleam@http@request:request(LTG)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(LVC)) -> LVK)) -> fun((gleam@http@request:request(LVC)) -> LVK).
