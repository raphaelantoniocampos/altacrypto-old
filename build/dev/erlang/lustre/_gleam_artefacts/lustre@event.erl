...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, QOC} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(QOC).
on(Name, Handler) ->
    lustre@attribute:on(Name, Handler).

-spec on_click(QOF) -> lustre@internals@vdom:attribute(QOF).
on_click(Msg) ->
    on(<<"click"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_down(QOH) -> lustre@internals@vdom:attribute(QOH).
on_mouse_down(Msg) ->
    on(<<"mousedown"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_up(QOJ) -> lustre@internals@vdom:attribute(QOJ).
on_mouse_up(Msg) ->
    on(<<"mouseup"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_enter(QOL) -> lustre@internals@vdom:attribute(QOL).
on_mouse_enter(Msg) ->
    on(<<"mouseenter"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_leave(QON) -> lustre@internals@vdom:attribute(QON).
on_mouse_leave(Msg) ->
    on(<<"mouseleave"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_over(QOP) -> lustre@internals@vdom:attribute(QOP).
on_mouse_over(Msg) ->
    on(<<"mouseover"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_out(QOR) -> lustre@internals@vdom:attribute(QOR).
on_mouse_out(Msg) ->
    on(<<"mouseout"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_keypress(fun((binary()) -> QOT)) -> lustre@internals@vdom:attribute(QOT).
on_keypress(Msg) ->
...
-spec on_keydown(fun((binary()) -> QOV)) -> lustre@internals@vdom:attribute(QOV).
on_keydown(Msg) ->
...
-spec on_keyup(fun((binary()) -> QOX)) -> lustre@internals@vdom:attribute(QOX).
on_keyup(Msg) ->
...
-spec on_focus(QPF) -> lustre@internals@vdom:attribute(QPF).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_blur(QPH) -> lustre@internals@vdom:attribute(QPH).
on_blur(Msg) ->
...
-spec on_input(fun((binary()) -> QOZ)) -> lustre@internals@vdom:attribute(QOZ).
on_input(Msg) ->
...
-spec on_check(fun((boolean()) -> QPB)) -> lustre@internals@vdom:attribute(QPB).
on_check(Msg) ->
...
-spec on_submit(QPD) -> lustre@internals@vdom:attribute(QPD).
on_submit(Msg) ->
    on(
        <<"submit"/utf8>>,
        fun(Event) ->
