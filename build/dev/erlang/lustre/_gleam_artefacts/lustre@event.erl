...
-spec on(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, TGT} |
        {error, list(gleam@dynamic:decode_error())})
) -> lustre@internals@vdom:attribute(TGT).
on(Name, Handler) ->
    lustre@attribute:on(Name, Handler).

-spec on_click(TGW) -> lustre@internals@vdom:attribute(TGW).
on_click(Msg) ->
    on(<<"click"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_down(TGY) -> lustre@internals@vdom:attribute(TGY).
on_mouse_down(Msg) ->
    on(<<"mousedown"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_up(THA) -> lustre@internals@vdom:attribute(THA).
on_mouse_up(Msg) ->
    on(<<"mouseup"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_enter(THC) -> lustre@internals@vdom:attribute(THC).
on_mouse_enter(Msg) ->
    on(<<"mouseenter"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_leave(THE) -> lustre@internals@vdom:attribute(THE).
on_mouse_leave(Msg) ->
    on(<<"mouseleave"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_over(THG) -> lustre@internals@vdom:attribute(THG).
on_mouse_over(Msg) ->
    on(<<"mouseover"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_mouse_out(THI) -> lustre@internals@vdom:attribute(THI).
on_mouse_out(Msg) ->
    on(<<"mouseout"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_keypress(fun((binary()) -> THK)) -> lustre@internals@vdom:attribute(THK).
on_keypress(Msg) ->
...
-spec on_keydown(fun((binary()) -> THM)) -> lustre@internals@vdom:attribute(THM).
on_keydown(Msg) ->
...
-spec on_keyup(fun((binary()) -> THO)) -> lustre@internals@vdom:attribute(THO).
on_keyup(Msg) ->
...
-spec on_focus(THW) -> lustre@internals@vdom:attribute(THW).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, fun(_) -> {ok, Msg} end).

-spec on_blur(THY) -> lustre@internals@vdom:attribute(THY).
on_blur(Msg) ->
...
-spec on_input(fun((binary()) -> THQ)) -> lustre@internals@vdom:attribute(THQ).
on_input(Msg) ->
...
-spec on_check(fun((boolean()) -> THS)) -> lustre@internals@vdom:attribute(THS).
on_check(Msg) ->
...
-spec on_submit(THU) -> lustre@internals@vdom:attribute(THU).
on_submit(Msg) ->
    on(
        <<"submit"/utf8>>,
        fun(Event) ->
