...
-spec component(list(lustre@internals@vdom:attribute(TLV))) -> lustre@internals@vdom:element(TLV).
component(Attrs) ->
...
-spec subscribe(binary(), fun((lustre@internals@patch:patch(TMI)) -> nil)) -> lustre@internals@runtime:action(TMI, lustre:server_component()).
...
-spec do_set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), TMY))
) -> lustre@effect:effect(TMY).
do_set_selector(Sel) ->
    lustre@effect:from(
        fun(_) ->
...
-spec set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), TMS))
) -> lustre@effect:effect(TMS).
set_selector(Sel) ->
    do_set_selector(Sel).
