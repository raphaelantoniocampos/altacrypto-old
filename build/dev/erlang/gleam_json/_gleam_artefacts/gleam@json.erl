...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JBG} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JBG} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JBC} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JBC} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JAY} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JAY} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(JBM), fun((JBM) -> json())) -> json().
...
-spec array(list(JBQ), fun((JBQ) -> json())) -> json().
