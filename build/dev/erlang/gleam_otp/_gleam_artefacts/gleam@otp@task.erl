...
-opaque task(MUQ) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(MUQ))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(MUR) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, MUR}.

-spec async(fun(() -> MUS)) -> task(MUS).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(MUW), integer()) -> {ok, MUW} | {error, await_error()}.
...
-spec await(task(MVA), integer()) -> MVA.
await(Task, Timeout) ->
...
-spec try_await_forever(task(MVC)) -> {ok, MVC} | {error, await_error()}.
...
-spec await_forever(task(MVG)) -> MVG.
await_forever(Task) ->
