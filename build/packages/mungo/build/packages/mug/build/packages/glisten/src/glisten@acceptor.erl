...
-type pool(HPN, HPO) :: {pool,
        glisten@socket:listen_socket(),
        fun((glisten@handler:loop_message(HPN), HPO, glisten@handler:connection(HPN)) -> gleam@otp@actor:next(glisten@handler:loop_message(HPN), HPO)),
        integer(),
        fun(() -> {HPO, gleam@option:option(gleam@erlang@process:selector(HPN))}),
        gleam@option:option(fun((HPO) -> nil)),
        glisten@socket@transport:transport()}.
                            ...
                            fun(Sock) ->
                                gleam@result:then(
                                    begin
                                        ...
                                        _pipe@3 = glisten@handler:start(_pipe@2),
                                        gleam@result:replace_error(
                                            _pipe@3,
                                            handler_error
                                        )
