-record(ssl, {
    ...
    negotiated_protocol :: fun((glisten@socket:socket()) -> {ok, binary()} |
        {error, binary()}),
    peername :: fun((glisten@socket:socket()) -> {ok,
    ...
    receive_timeout :: fun((glisten@socket:socket(), integer(), integer()) -> {ok,
            bitstring()} |
        {error, glisten@socket:socket_reason()}),
    send :: fun((glisten@socket:socket(), gleam@bytes_builder:bytes_builder()) -> {ok,
            nil} |
        {error, glisten@socket:socket_reason()}),
    ...
    shutdown :: fun((glisten@socket:socket()) -> {ok, nil} |
        {error, glisten@socket:socket_reason()}),
    socket_info :: fun((glisten@socket:socket()) -> gleam@dict:dict(gleam@erlang@atom:atom_(), gleam@dynamic:dynamic_()))
}).
