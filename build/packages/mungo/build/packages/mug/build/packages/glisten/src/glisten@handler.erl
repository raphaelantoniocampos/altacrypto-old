...
-type message(HGR) :: {internal, internal_message()} | {user, HGR}.

-type loop_message(HGS) :: {packet, bitstring()} | {custom, HGS}.

-type loop_state(HGT, HGU) :: {loop_state,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(HGT)),
        glisten@socket@transport:transport(),
        HGU}.

-type connection(HGV) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@socket@transport:transport(),
        gleam@erlang@process:subject(message(HGV))}.

-type handler(HGW, HGX) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(HGW), HGX, connection(HGW)) -> gleam@otp@actor:next(loop_message(HGW), HGX)),
        fun(() -> {HGX, gleam@option:option(gleam@erlang@process:selector(HGW))}),
        gleam@option:option(fun((HGX) -> nil)),
        glisten@socket@transport:transport()}.

-spec start(handler(HHH, any())) -> {ok,
        gleam@erlang@process:subject(message(HHH))} |
    ...
    gleam@otp@actor:start_spec(
        {spec,
            fun() ->
                ...
                {ready,
                    {loop_state,
                        (erlang:element(9, erlang:element(6, Handler)))(
            ...
            fun(Msg, State) ->
                ...
                case Msg of
                    {internal, tcp_closed} ->
                        case (erlang:element(4, erlang:element(5, State)))(
                            erlang:element(3, State)
                        ) of
                            {ok, nil} ->
                    ...
                    {internal, ready} ->
                        ...
                        _pipe@15 = (erlang:element(6, erlang:element(5, State)))(
                            _pipe@14
                        ),
